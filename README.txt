...
The per-residue RMSF script determines the C-alpha RMSF between all structures within one conformational ensemble. As input, the script takes a folder containing individual PDB files for each structural conformer of the ensemble. For RMSF calculation, a centroid structure, which is determined as the PDB structure with the lowest RMSD to the ensemble mean structure, is selected as the reference for alignment of all other PDB structures within the folder. The script will output an Excel file listing each residue compared and the calculated RMSF value. Note that this script is for one ensemble, and its output is intended to be used as input for the global delta RMSF determination script, which compares the RMSF of two ensembles. Folder/file paths must be designated in the code. Two example conformational ensemble input folders are available in the “Input Examples using PDB ID 1SRB” folder.
